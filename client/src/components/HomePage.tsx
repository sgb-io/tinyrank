...
export function HomePage({ session, setSession, navigate }: HomePageProps) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (items.length < 1) { setCreateError('At least 1 item required'); return; }
    ...
    try {
      ...
      if (!res.ok) { setCreateError(data.error || 'Failed to create poll'); return; }
      setSession({ ...session, pollsCreated: [...session.pollsCreated, data.code] });
        ...
        <div className="card">
          ...
          {session.pollsCreated.length > 0 && (
            <div className="my-polls">
              <h3>Your Polls</h3>
              <ul>
                {session.pollsCreated.map(code => (
                  <li key={code}>
                    <button className="poll-link" onClick={() => navigate(`/poll/${code}`)}>
                      /poll/{code}
                    </button>
                  </li>
                ))}
